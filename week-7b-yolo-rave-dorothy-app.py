...
from util.yolo_draw_utils import draw_skeleton
from util.latent_util import create_latent_interp, clamp
from util.rave_download import download_pretrained_rave
...
class MySketch:
    ...
    def setup(self):
        print("setup")
        # Turn off autofocus
        rave_model_dir = 'demo/ckpt/rave'

        self.camera.set(cv2.CAP_PROP_AUTOFOCUS, 0)
        download_pretrained_rave(rave_model_dir)
        
        # Choose from:
        # - "isis.ts", rave_latent_dim = 4
        # - "sol_ordinario_fast.ts", rave_latent_dim = 4
        # - "darbouka_onnx.ts", rave_latent_dim = 4
        self.rave_latent_dim = 4
        
        self.rave_id = dot.music.start_rave_stream(os.path.join(rave_model_dir, "isis.ts"), latent_dim=self.rave_latent_dim)   
...
# TASKS
#
# Task 3: Can you change the RAVE model loaded in from 'vintage.pt' To one of the other options
#
# Task 4: Can you change the keypoint controlling the generation from the y position of the left wrist to the x position of the right wrist?
#
# Task 5: Can you use two inputs to control the generation, to do this you will need to:
